...
#        model = WhisperModel(modelsize, device="cpu", compute_type="int8") #, download_root="faster-disk-cache-dir/")
    ...
    def transcribe(self, audio, init_prompt=""):
        # 从配置文件中读取repetition_penalty和no_repeat_ngram_size参数
        import yaml
        import os
        
        config_path = "whisper_code/config.yaml"
        if os.path.exists(config_path):
            with open(config_path, 'r') as file:
                config = yaml.safe_load(file)
            
            repetition_penalty = config.get('repetition_penalty', 3.0)
        ...
        logger.info(f"Using repetition_penalty={repetition_penalty}, no_repeat_ngram_size={no_repeat_ngram_size}")
        ...
        segments, info = self.model.transcribe(
            ...
            repetition_penalty=repetition_penalty,  # 使用配置文件中的值
            no_repeat_ngram_size=no_repeat_ngram_size,  # 使用配置文件中的值
            **self.transcribe_kargs
        )
